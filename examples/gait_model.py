...
ref_pos_knee_deg = np.array([-0.6, 1.1, 3, 5.1, 7.4, 9.8, 12.1, 14, 15.4, 16.2, 16.3, 15.9, 15.2, 14.2, 13.2, 12.3, 11.4, 10.7, 10.1, 
                    ...
                    40, 34.5, 28.6, 22.6, 16.5, 10.9, 5.8, 1.8, -1.1, -2.6, -3, -2.3])
...
with osl:
    ...
    ki = 0
    kd = 0
    osl.ankle.set_position_gains(kp=kp, ki=ki, kd=kd)

    # Set the initial output position for the joints
    osl.ankle.set_output_position(0)
    osl.update()
    ...
    for t in osl.clock:
        ...
        if t - last_update_time >= 0.015:
            ...
            time_list.append(t)
            desired_ankle_list.append(desired_ankle)
            actual_ankle_list.append(osl.ankle.output_position)
            error_ankle_list.append(desired_ankle - osl.ankle.output_position)
            #desired_knee_list.append(desired_knee)
            #actual_knee_list.append(osl.knee.output_position)
            #error_knee_list.append(desired_knee - osl.knee.output_position)
            
            # Print the current time and joint positions
            print(
                "Time: {:+.3f}, Ankle Desired: {:+.3f} rad, Ankle Actual: {:+.3f} rad".format(
                t,
                desired_ankle,
